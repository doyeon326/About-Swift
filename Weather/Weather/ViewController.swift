...
class ViewController: UIViewController {
    ...
    func getCurrentWeather(cityName: String) {
        guard let url = URL(string: "https://api.openweathermap.org/data/2.5/weather?q=\(cityName)&appid=\(Constants.WeatherAPI)") else { return }
        let session = URLSession(configuration: .default)
        session.dataTask(with: url, completionHandler: { [weak self] data, response, error in
            /*
             data = 서버에서 응답받은 데이터
             response = 응답코드
             error = 요청이 실패할경우 error, 성공할경우 nil
             */
